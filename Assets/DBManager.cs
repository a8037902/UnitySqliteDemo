...
public class DBManager : MonoBehaviour
{
    ...
    public void CreateTable<T>()
    {
        ...
        var type = typeof(T);
        string sql = "select count(*) from sqlite_master where type = 'table' and name = '" + type.Name + "'";
        ...
        if (reader.Read())
        {
            if (reader.GetInt32(0) == 0)
            {
                ...
                for (int i = 0; i < fields.Length; i++)
                {
                    ...
                    object[] obj = fields[i].GetCustomAttributes(false);
                    if (obj != null && obj.Length > 0)
                    {
                        if (((FieldAttribute)obj[0]).PrimaryKey)
                        {
                            fieldAttributes = " PRIMARY KEY";
                            if (((FieldAttribute)obj[0]).AutoIncrement)
                            {
                                fieldAttributes += " AUTOINCREMENT";
                            }
                        }
                    }
                    sql += " [ " + fields[i].Name + "] " + CS2DB(fields[i].FieldType) + fieldAttributes + ",";
                }
                sql = sql.TrimEnd(',') + ")";
                ExecuteQuery(sql);
            }
